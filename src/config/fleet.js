...
//
// TODO（本番前・必須）: M5Stack の IP を固定化する。
//   現状の host(192.168.10.11〜15) は仮。DHCP のままだと M5 の IP が再起動で
//   変わり、ここと食い違って届かなくなる。本番ネットワーク確定後にどちらかで固定:
//     方法A: 会場ルーターで DHCP 予約（MAC→IP を5台分登録）→ ここの host を一致させる
//     方法B(推奨): ファーム側で SUITCASE_ID から固定IP を自動設定
//                 （192.168.x.(10+ID) → #1=.11 #2=.12… でここと自動一致）
//   詳細は Notion「実装サマリー: 中央サーバー → OSC → M5Stack」TODO C を参照。
// ============================================================================
