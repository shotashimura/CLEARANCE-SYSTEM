// OpenSky フライト取得の「純粋ロジック」コア。
// import.meta.env / Vite 依存・fetch 先 URL を含まないため、
// ブラウザ（src/lib/flights.js）とサーバー（server/flights.js）の双方から共用できる。
// fetch 自体はブラウザ・Node ともにグローバルに存在する前提（Node 18+）。
...
async function fetchRegion(baseUrl, box, signal) {
  const url =
...
export function enrich(raw) {
  ...
  const status = raw.velocity_ms && raw.velocity_ms > 220 ? "ON_TIME" : "DELAYED";
  return {
    flight: raw.callsign,
...
/**
 * 指定の baseUrl（ブラウザ=プロキシ /opensky、サーバー=直URL）から
 * 5地域を並列取得し、1便ずつ enrich する。失敗地域はモックで埋める。
 */
export async function fetchFiveFlights(baseUrl, { timeoutMs = 8000 } = {}) {
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), timeoutMs);
  try {
    const results = await Promise.all(
      BOUNDING_BOXES.map((box) =>
        fetchRegion(baseUrl, box, controller.signal)
          .then(enrich)
          .catch(() => null)
      )
    );
    return results.map((r, i) => r ?? MOCK_FLIGHTS[i]);
  } catch {
    return MOCK_FLIGHTS;
  } finally {
    clearTimeout(timer);
...
export const MOCK_FLIGHTS = [
  {
    flight: "NH205", callsign: "ANA205", origin: "HND", destination: "FRA",
    origin_country: "Japan", region: "east_asia", status: "ON_TIME",
    altitude_m: 11200, velocity_ms: 245, delay_minutes: 0, congestionLevel: "HIGH",
    routeHistory: "過去6ヶ月で同ルート類似プロファイル インシデント記録なし",
    metalAlert: true, passengerCount: 189, source: "mock",
  },
  {
    flight: "LH717", callsign: "DLH717", origin: "FRA", destination: "HND",
    origin_country: "Germany", region: "europe", status: "DELAYED",
    altitude_m: 10800, velocity_ms: 232, delay_minutes: 12, congestionLevel: "MEDIUM",
    routeHistory: "同ルート過去3ヶ月：遅延2件、インシデント1件記録",
    metalAlert: false, passengerCount: 245, source: "mock",
  },
  {
    flight: "EK316", callsign: "UAE316", origin: "DXB", destination: "CDG",
    origin_country: "United Arab Emirates", region: "middle_east", status: "ON_TIME",
    altitude_m: 11800, velocity_ms: 258, delay_minutes: 0, congestionLevel: "LOW",
    routeHistory: "インシデント記録なし", metalAlert: false, passengerCount: 312, source: "mock",
  },
  {
    flight: "AA101", callsign: "AAL101", origin: "JFK", destination: "LHR",
    origin_country: "United States", region: "north_america", status: "ON_TIME",
    altitude_m: 11000, velocity_ms: 248, delay_minutes: 0, congestionLevel: "MEDIUM",
    routeHistory: "—", metalAlert: false, passengerCount: 220, source: "mock",
  },
  {
    flight: "QF7", callsign: "QFA7", origin: "SYD", destination: "LAX",
    origin_country: "Australia", region: "oceania", status: "DELAYED",
    altitude_m: 10500, velocity_ms: 228, delay_minutes: 22, congestionLevel: "LOW",
    routeHistory: "—", metalAlert: false, passengerCount: 270, source: "mock",
  },
];
